                ...
                <div class="row">
                    ...
                    <div class="box-2">
                        <Form @submit="orderNow" :validation-schema="schema" :initial-values="initialValues" v-slot="{ errors }">
                            <Field name="name" as="input" type="text" placeholder="Enter Name" class="mail_text order_text" v-model='initialValues.name' readonly />
...
export default {
    ...
    setup() {
        ...
        const getProductImage = (id) => `${import.meta.env.VITE_IMAGE_URL}/${productDetails.value[id-1]?.image[0]}` || "Unknown Product";
        ...
        const decrement = (id) => {
            ...
            if (product && product.qnty > 1) {
                ...
                Swal.fire({
                    ...
                    sessionStorage.setItem("cart", JSON.stringify(cartDetails.value.filter((item) => item.id !== id)));
        ...
        const displayRazorpay = async (newValue) => {
            ...
            const result = await axios.post(`${import.meta.env.VITE_API_URL}/addorder`, newValue);
            ...
            const options = {
                ...
                handler: async function (response) {
                    ...
                    const result = await axios.post(`${import.meta.env.VITE_API_URL}/success`, data);
